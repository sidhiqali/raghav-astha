...
**Cover.** The terrace backdrop with the studio's own square envelope artwork on the pedestal (`assets/envelope-red.webp`: the burgundy envelope from the Tanvi & Nishant project, cropped to its paper and made symmetric by `work/symmetrize.py`; its seal already reads R A. An ivory recolour by `work/recolor.py` is kept in `work/assets-src/` if wanted). It is sliced exactly as that site slices it: the pocket is the bottom half, the flap is the band from 6.4% to 49.2% with rounded lower corners, and the seal painted across that edge cracks naturally when the flap lifts. Behind the flap is a lined champagne back; inside is a burgundy pocket with the cream card, names in burgundy. Every keyframe matches the reference (flap over .9s from .3s, card rise 1s from 1.1s, envelope travel 2.75s, fade at 2.1s). The cover goes position-fixed while opening so the invitation renders underneath and the two crossfade at 2.75 seconds. Reduced motion skips straight to the invitation; "The beginning" in the header returns to the sealed envelope.
...
**Invitation.** Gold-frame stationery flatlay with the lace doily copy, one polaroid of the couple (`assets/couple.webp`, a resized copy of the supplied photo, in colour, no retouching). A small floating button at the bottom right of the screen pauses and resumes the song once the invitation is open.

**Type.** Pinyon Script for names, Cormorant Garamond for text, Cinzel for the small caps labels, in a near-black warm ink.

**Song.** `assets/song.m4a` is the supplied "Jahaan" by Lost Stories, trimmed to start at 0:29 with a two second fade-in, AAC 96 kbps, about 2.2 MB. It starts on the seal tap (the gesture browsers require for audio), loops, and the floating button toggles it. It is not preloaded, so it costs nothing until the envelope is opened.
